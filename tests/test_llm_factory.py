...
class TestLlmFactory:
    ...
    def test_create_openai_client_uses_default_key(self):
        ...
        try:
            client = llm_factory.create_openai_client(
                api_key="test-key"
            )
            assert client is not None
        except Exception:
    ...
    def test_embedding_model_constant_exists(self):
        ...
        assert (
            llm_factory.EMBEDDING_MODEL == "text-embedding-3-small"
        )
    ...
    def test_create_embedding_function(
        self, mock_oai_client, mock_embedding_fn
    ):
        """Test creating embedding function."""
        api_key = "test-key-12345"
        embedding_fn = llm_factory.create_embedding_function(
            api_key=api_key
        )
    ...
    @patch("src.providers.llm_factory.OpenAIEmbeddingFunction")
    @patch("src.providers.llm_factory.create_openai_client")
    def test_create_embedding_function_sets_client(
        self, mock_oai_client, mock_embedding_fn_class
    ):

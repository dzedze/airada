...
class TestIngestData:
    ...
    def test_build_document_handles_missing_title(self):
        ...
        row = pd.Series(
            {"title": None, "abstract": "Test abstract"}
        )
    ...
    def test_build_document_handles_missing_abstract(self):
        ...
        row = pd.Series({"title": "Test Title", "abstract": None})
    ...
    def test_ingest_validates_required_columns(
        ...
        mock_df = pd.DataFrame(
            {"wrong_col1": ["value1"], "wrong_col2": ["value2"]}
        )
        mock_read_csv.return_value = mock_df
        ...
        mock_df = pd.DataFrame(
            {
                ...
                "abstract": [
                    f"Abstract {i}" for i in range(num_papers)
                ],

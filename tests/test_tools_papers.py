...
class TestPapersTools:
    ...
    def test_get_collection_initializes_client(self, mock_embedding_fn, mock_client):
    ...
    @patch('chromadb.PersistentClient')
    @patch('src.providers.llm_factory.create_embedding_function')
    def test_get_collection_caches_result(self, mock_embedding_fn, mock_client):
    ...
    @patch('chromadb.PersistentClient')
    @patch('src.providers.llm_factory.create_embedding_function')
    def test_get_collection_handles_connection_error(self, mock_embedding_fn, mock_client):
    ...
    @patch('chromadb.PersistentClient')
    @patch('src.providers.llm_factory.create_embedding_function')
    def test_search_ai_papers_with_available_db(self, mock_embedding_fn, mock_client):
        ...
        result = tools_papers.search_ai_papers.invoke({"query": "multi-agent learning"})
    ...
    @patch('chromadb.PersistentClient')
    @patch('src.providers.llm_factory.create_embedding_function')
    def test_search_ai_papers_without_db(self, mock_embedding_fn, mock_client):
        """Test searching papers when ChromaDB is not available."""
        # Reset global collection state
        tools_papers._collection = None

        mock_client_instance = MagicMock()
        mock_client_instance.get_collection.side_effect = Exception("DB not found")
        mock_client.return_value = mock_client_instance
        mock_embedding_fn.return_value = MagicMock()

        result = tools_papers.search_ai_papers.invoke({"query": "test query"})
    ...
    @patch('chromadb.PersistentClient')
    @patch('src.providers.llm_factory.create_embedding_function')
    def test_search_ai_papers_respects_top_k(self, mock_embedding_fn, mock_client):
        ...
        mock_collection.query.return_value = {
            ...
            "metadatas": [[
                {"title": f"Paper {i}", "arxiv_id": f"id{i}", "url_abs": f"url{i}", "url_pdf": f"pdf{i}"}
                for i in range(5)
            ]],
        ...
        tools_papers.search_ai_papers.invoke({"query": "test query"})
    ...
    @patch('chromadb.PersistentClient')
    @patch('src.providers.llm_factory.create_embedding_function')
    def test_search_ai_papers_formats_output(self, mock_embedding_fn, mock_client):
    ...
    @patch('chromadb.PersistentClient')
    @patch('src.providers.llm_factory.create_embedding_function')
    def test_search_ai_papers_handles_missing_metadata(self, mock_embedding_fn, mock_client):
    ...
    @patch('chromadb.PersistentClient')
    @patch('src.providers.llm_factory.create_embedding_function')
    def test_search_ai_papers_includes_relevance_score(self, mock_embedding_fn, mock_client):
        ...
        mock_collection.query.return_value = {
            ...
            "distances": [[0.25]],  # Distance = 0.25, so score = 1 - 0.25 = 0.75
        }
